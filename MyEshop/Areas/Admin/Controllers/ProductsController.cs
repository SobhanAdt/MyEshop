...
namespace MyEshop.Areas.Admin.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "ProductID,ProductTitle,ShortDescription,Text,Price,CreateDate,ImageName")] Products products, List<int> selectedGroups, HttpPostedFileBase imageProduct, string tags)
        {
            if (ModelState.IsValid)
            {
                ...
                products.ImageName = "not-available.png";
                if (imageProduct != null && imageProduct.IsImage())
                {
                    products.ImageName = Guid.NewGuid() + Path.GetExtension(imageProduct.FileName);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ProductID,ProductTitle,ShortDescription,Text,Price,CreateDate,ImageName")] Products products, List<int> selectedGroups, HttpPostedFileBase imageProduct, string tags)
        {
            if (ModelState.IsValid)
            {
                if (imageProduct != null && imageProduct.IsImage())
                {
                    if (products.ImageName != "not-available.png")
                    {
        ...
        public ActionResult ProductFeatures(Product_Features features)
        {
            if (ModelState.IsValid)
            {
                db.Product_Features.Add(features);
                db.SaveChanges();
            }
